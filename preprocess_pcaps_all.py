...
def preprocess_pcap(pcap_path, output_dir):
    ...
    try:
        with ReaderClass(pcap_path) as pcap_reader:
            ...
            for i, pcap_data in enumerate(pcap_reader):
                if i >= 1000000: # Increased packet limit for search
                    break
                ...
                # EXTENDED: Increased limit from 128 to 1024
                if len(flow['packets']) < 1024:
    ...
    for flow_id, flow_data in flows.items():
        # Keep same filter: need at least 3 packets to be interesting
        if len(flow_data['packets']) < 3: continue
        
        # 1. Temporal - Full 1024 packets
...
if __name__ == '__main__':
    output_directory = 'data/processed_all'
