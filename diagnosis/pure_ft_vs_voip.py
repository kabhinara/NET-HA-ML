...
def extract_micro_features(temp_tensor, spat_tensor):
    ...
    for i in range(temp_tensor.shape[0]):
        ...
        # 2. Micro-Burst Ratio (% of packets larger than 1200 bytes for pure FT)
        large_pkt_ratio = np.sum(sizes > 1200) / 1024.0
        
        # 3. Direction Flip Rate (How often it switches from send to receive)
        flips = np.sum(directions[:-1] != directions[1:]) / 1024.0
        
        # 4. Spatial/Payload Sparsity
...
def test_pure_ft_vs_voip():
    data_dir = 'data/processed'
    X, y = [], []
    
    print("Extracting micro-features for Pure FT vs VOIP...")
    for f in os.listdir(data_dir):
        ...
        # Class 1: PURE File Transfer (No Skype)
        if "FT" in true_label and "skype" not in filename:
            label = 1
        # Class 0: ANY VOIP
        elif "VOIP" in true_label:
            label = 0
        else:
            continue # Ignore chat, streaming, browsing, etc.
    ...
    print(f"Extracted {len(y)} flows (Pure FT: {np.sum(y==1)}, VOIP: {np.sum(y==0)})")
    ...
    feature_names = ["Jitter (IAT StdDev)", "Large Pkt Ratio (>1200B)", "Direction Flip Rate", "Payload Sparsity", "Max Packet Size"]
