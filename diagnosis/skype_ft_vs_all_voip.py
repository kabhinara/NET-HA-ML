...
def extract_targeted_features(temp_tensor):
    features = []
    # Using only temporal tensor as we only need sizes and directions
    for i in range(temp_tensor.shape[0]):
        ...
        # 3. Direction Flip Rate (Asymmetry metric)
        flips = np.sum(directions[:-1] != directions[1:]) / 1024.0
        
        # 4. Direction Ratio Fwd/Bwd (Asymmetry metric)
...
def test_skype_ft_vs_all_voip():
    data_dir = 'data/processed'
    X, y = [], []
    
    print("Extracting targeted features (Size & Asymmetry) for Skype FT vs ALL VOIP...")
    ...
    for f in os.listdir(data_dir):
        ...
        feats = extract_targeted_features(data['temporal'])
        X.append(feats)
    ...
    print("\nTraining Random Forest Classifier on ONLY Size and Asymmetry features...")
