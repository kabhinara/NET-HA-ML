...
def test_skype_ft_vs_pure_voip():
    data_dir = 'data/processed'
    X_skype_ft, X_pure_voip = [], []
    
    print("Extracting 15 comprehensive features for Skype FT vs Pure VOIP...")
    ...
    for f in os.listdir(data_dir):
        ...
        if is_skype and is_ft:
            X_skype_ft.append(feats)
        elif not is_skype and is_voip:
            X_pure_voip.append(feats)

    X_skype_ft = np.vstack(X_skype_ft)
    X_pure_voip = np.vstack(X_pure_voip)
    
    print(f"\nOriginal counts -> Skype FT: {len(X_skype_ft)}, Pure VOIP: {len(X_pure_voip)}")
    
    # Balance the dataset (Undersample Pure VOIP to match Skype FT)
    min_samples = min(len(X_skype_ft), len(X_pure_voip))
    X_skype_ft_b = resample(X_skype_ft, replace=False, n_samples=min_samples, random_state=42)
    X_pure_voip_b = resample(X_pure_voip, replace=False, n_samples=min_samples, random_state=42)
    
    print(f"Balanced counts -> Skype FT: {len(X_skype_ft_b)}, Pure VOIP: {len(X_pure_voip_b)}")
    
    X = np.vstack([X_skype_ft_b, X_pure_voip_b])
    # Label 1 = Skype FT, Label 0 = Pure VOIP
    y = np.array([1] * len(X_skype_ft_b) + [0] * len(X_pure_voip_b))
    ...
    print(f"                 Predicted Pure VOIP     Predicted Skype FT")
