...
def test_skype_diff():
    ...
    print("Extracting 15 comprehensive features for Skype FT vs Skype VOIP...")
    ...
    print(f"\nOriginal counts -> Skype FT: {len(X_ft)}, Skype VOIP: {len(X_voip)}")
    
    # Undersample Skype VOIP to match Skype FT for a perfectly balanced test
    X_voip_balanced = resample(X_voip, replace=False, n_samples=len(X_ft), random_state=42)
    
    print(f"Balanced counts -> Skype FT: {len(X_ft)}, Skype VOIP: {len(X_voip_balanced)}")
