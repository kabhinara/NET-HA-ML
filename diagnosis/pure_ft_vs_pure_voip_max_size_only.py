...
def test_pure_ft_vs_pure_voip_max_size_only():
    data_dir = 'data/processed'
    X, y = [], []
    
    print("Extracting ONLY Max Size feature for Pure FT vs Pure VOIP...")
    ...
    for f in os.listdir(data_dir):
        ...
        # We want: Class 1 = Pure FT (not skype), Class 0 = Pure VOIP (not skype)
        if not is_skype and is_ft:
            label = 1
            pure_ft_count += data['temporal'].shape[0]
        elif not is_skype and is_voip:
            label = 0
            pure_voip_count += data['temporal'].shape[0]
        else:
            continue
    ...
    print(f"Extracted {len(y)} flows (Pure FT: {pure_ft_count}, Pure VOIP: {pure_voip_count})")
...
if __name__ == "__main__":
    test_pure_ft_vs_pure_voip_max_size_only()
