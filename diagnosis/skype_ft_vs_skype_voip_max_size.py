...
def test_skype_ft_vs_skype_voip_max_size():
    data_dir = 'data/processed'
    X, y = [], []
    
    print("Extracting ONLY Max Size feature for Skype FT vs Skype VOIP...")
    ...
    for f in os.listdir(data_dir):
        ...
        # We want: Class 1 = Skype FT, Class 0 = Skype VOIP
        if is_skype and is_ft:
            label = 1
            skype_ft_count += data['temporal'].shape[0]
        elif is_skype and is_voip:
            label = 0
            skype_voip_count += data['temporal'].shape[0]
        else:
            continue
    ...
    print(f"Extracted {len(y)} flows (Skype FT: {skype_ft_count}, Skype VOIP: {skype_voip_count})")
...
if __name__ == "__main__":
    test_skype_ft_vs_skype_voip_max_size()
